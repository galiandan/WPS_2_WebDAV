...
默认版本为 `1.0.8`。发布构建应注入提交号和 UTC 构建时间：

```sh
CGO_ENABLED=0 go build -trimpath \
  -ldflags "-s -w -X main.version=1.0.8 -X main.commit=$(git rev-parse HEAD) -X main.buildTime=$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  -o /tmp/wps-adapter ./cmd/wps-adapter
```

```sh
/tmp/wps-adapter --version
# 1.0.8 commit=<commit> build_time=<UTC时间>
```
