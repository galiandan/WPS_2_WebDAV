...
func TestVersionReportsTheBuildSummary(t *testing.T) {
	...
	if !strings.HasPrefix(line, "1.0.0 commit=") || !strings.Contains(line, " build_time=") {
...
func TestServeAnswersHealthzBeforeStop(t *testing.T) {
	...
	response.Body.Close()
	want := `{"status":"ok","service":"wps-enterprise-adapter","version":"1.0.0","network_calls":"on-demand"}`
