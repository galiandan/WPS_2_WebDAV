...
func TestVersionReportsTheBuildSummary(t *testing.T) {
	...
	if !strings.HasPrefix(line, "0.9.105 commit=") || !strings.Contains(line, " build_time=") {
...
func TestServeAnswersHealthzBeforeStop(t *testing.T) {
	...
	response.Body.Close()
	want := `{"status":"ok","service":"wps-enterprise-adapter","version":"0.9.105","network_calls":"on-demand"}`
