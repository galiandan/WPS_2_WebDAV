...
// friends) through suffix_map, peel one case-sensitive encoding suffix,
...
func GuessMimeType(name string) string {
	...
	for {
		...
		base, ext = pythonSplitExt(base + rewritten)
	}
	// encodings_map is case sensitive; the encoding itself is discarded
	// because the server only consumes the type, but the peel still
	// changes the extension that the type lookup sees (x.gz → "" not
	// ".gz", so it falls back to octet-stream like Python).
	if _, ok := mimeEncodingsTable[ext]; ok {
		base, ext = pythonSplitExt(base)
	}
	if typ, ok := mimeTypesTable[strings.ToLower(ext)]; ok {
		return typ
	}
	return "application/octet-stream"
}

// pythonSplitExt mirrors posixpath.splitext: the extension starts at the
// last dot after the last separator, but leading dots in the file name
// never count, so ".hidden" and "x." have no extension.
func pythonSplitExt(p string) (string, string) {
	sepIndex := strings.LastIndexByte(p, '/')
	dotIndex := strings.LastIndexByte(p, '.')
	if dotIndex > sepIndex {
		filenameIndex := sepIndex + 1
		for filenameIndex < dotIndex && p[filenameIndex] == '.' {
			filenameIndex++
		}
...
// mimeSuffixTable mirrors mimetypes.suffix_map (keys compared lowercased).
var mimeSuffixTable = map[string]string{
	".svgz": ".svg.gz",
	".tgz":  ".tar.gz",
	".taz":  ".tar.gz",
	".tz":   ".tar.gz",
...
// mimeEncodingsTable mirrors mimetypes.encodings_map (case sensitive).
var mimeEncodingsTable = map[string]string{
	".gz":  "gzip",
	".Z":   "compress",
	".bz2": "bzip2",
	".xz":  "xz",
...
// mimeTypesTable mirrors CPython 3.14 mimetypes.guess_type's effective
// strict table: _types_map_default merged with the gate machine's
// /etc/mime.types (the environment the contract goldens were captured
// in); generated mechanically so the mapping stays identical.
var mimeTypesTable = map[string]string{
