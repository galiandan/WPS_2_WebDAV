...
网页文件管理器在当前目录成功加载后，会另外在浏览器内存中预取最多 8 个直接子文件夹，最多同时执行 1 个目录请求，并缓存 30 秒。进入已经有缓存的目录时，页面先绘制缓存内容，再后台刷新远端目录；首次访问没有缓存时才展示骨架屏。状态检查只在后台低频执行，不会成为目录导航的前置请求。该缓存只保存目录条目元数据，不保存文件正文或凭据；它与服务端短 TTL 元数据缓存分离，刷新、写操作和重新连接时必须防止旧结果污染当前目录。
...
4. `MultiSpace` 将所有选中的 WPS 空间挂载为网页虚拟目录 `/A/`、`/B/`，并使用短 TTL 元数据缓存减少重复列表请求；`DAVView` 再把 `/dav/` 请求映射到唯一配置的子树，例如 `/A/web`，因此 WebDAV 不会越过该目录访问其他空间。
...
8. `/api/v1/status` 使用缓存的只读 `islogin` 预检和一次最小根目录列表，网页据此区分进程健康与 WPS 会话状态；它只负责后台状态徽标，目录导航以实际列表请求为准。
9. 网页的云盘显示名称通过同源、网页会话或 Basic Auth 保护的设置接口写入本机状态文件，不依赖 WPS 请求。
10. 网页通过受保护的更新接口查询国内加速的 GitHub Release；一键更新下载对应架构二进制、执行版本检查、原子替换 `runtime/` 文件并重启进程，不触碰配置和凭据。
