...
使用 root 创建目录和文件。Cookie 文件写入浏览器当前会话的完整 Cookie 行，CSRF 文件写入 `csrf` Cookie 的值；如果不单独创建 CSRF 文件，程序也会尝试从 Cookie 行中提取名为 `csrf` 的值。
...
如果返回 `503` 且 `upstream_status` 是 `401`，说明 WPS 会话失效或被撤销。若配置了刷新助手，适配器会先按上面的规则调用它；仍然失败时，重新在本机浏览器建立本人账号会话，然后只更新 VPS 上的 secret 文件。当前项目没有实现未经抓包确认的登录、SSO 或验证码流程。
