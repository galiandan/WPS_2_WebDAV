...
网页首页发现新版本后可以直接点击“立即更新”。服务会在 `runtime/` 内完成预编译二进制的下载、版本检查、原子替换和重启；更新代码后也可以重新安装 service 文件、执行 `systemctl daemon-reload` 和 `systemctl restart wps-adapter`，再检查 `/healthz`。回滚只恢复代码和非秘密配置；不要从 Git 或备份中恢复旧 Cookie。
...
从 1.2.0 起，后台复制/移动/删除记录默认保存在网页设置文件同目录的 `tasks.json`。
可通过 `WPS_TASKS_FILE` 指定绝对路径；父目录必须允许服务用户原子创建/替换文件，文件权限为 `0600`。
Docker 和 systemd 安装的默认私有配置目录已提供持久化和写权限；自定义路径需要同步配置挂载和写权限。
文件包含任务路径、结果和内部身份绑定信息，不包含 WPS Cookie 或下载签名地址，不应公开。

若任务状态损坏或启动时不可写，服务拒绝启动并提示修复状态文件；运行中落盘失败则停止启动新任务。
服务重启将旧的排队/执行中任务标记为中断，不自动重放。查看远端结果后，可在任务中心重试明确未开始的项目。
已经开始但结果未知的项目不提供自动重试；成功项目也不会被旧任务重试再次执行。


### 成员账号与登录安全状态

从 1.4.0 起，`users.json` 默认保存于网页设置同目录，可用 `WPS_USERS_FILE` 指定。
文件以 `0600` 原子持久化，保存随机盐、PBKDF2 密码验证值和目录授权，不保存成员明文密码。
安装管理员仍来自安装器配置的凭据文件。先以管理员登录，再在设置中创建成员并选择具体空间或目录。

成员安全设置保存在同目录的 `auth-<随机账号ID>.json`；管理员原来的 `auth-settings.json` 不变。
这些文件与任务记录一样必须保留服务用户所有权、私有权限和 Docker 持久化挂载，不应公开。
管理员修改成员目录、权限、密码或停用账号后，旧会话需要重新登录；后台任务再次执行前重新验证授权。
