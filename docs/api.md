...
登录助手选择多个 WPS 空间时，适配器会在 `/dav/` 和网页根目录下创建对应的虚拟空间文件夹，例如 `/dav/学校云盘/`。空间文件夹只用于路由，不会在 WPS 中创建新目录；跨空间 `MOVE`、`COPY` 和直接对根目录执行写操作会被拒绝。旧版只保存单个 `group_id/root_id` 的工作区文件仍可读取。
