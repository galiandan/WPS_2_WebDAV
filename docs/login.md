...
5. 选定空间后，脚本会逐个显示该空间可访问的文件夹。输入 `0` 使用当前目录，输入文件夹序号进入子目录，输入 `b` 返回上一级。这里选择的目录就是 WebDAV 的 `/dav/` 根目录；脚本会把文件夹 ID 和显示路径一起保存。
6. 选定文件夹后，脚本会逐个验证目标目录可读，再继续同步；验证失败不会覆盖 VPS 上已有凭据。
...
省略 `--workspace-url` 时，脚本会在关闭浏览器后让你为每个已选空间选择存储文件夹。默认输入 `0` 映射该空间根目录；选择子文件夹后，WebDAV 只从该文件夹开始显示。WPS 自动恢复的旧浏览器文件夹不会被自动使用。选择多个空间时，每个空间名称会成为根目录下的虚拟文件夹；每个空间可以分别映射到自己的目标文件夹。

之后也可以直接在网页右上角“设置”中修改。网页选择器会从每个 WPS 空间的原始根目录开始浏览，即使当前已经映射到子文件夹，也可以切换到同级或其他子文件夹。保存后 `/dav/` 地址不变，文件不会被移动或复制，只有适配器的映射位置改变。
