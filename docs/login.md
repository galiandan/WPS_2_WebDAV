...
4. 登录完成后，脚本会先获取全部可见企业空间及其名称，然后自动关闭临时浏览器；多个空间时只能输入一个序号，不能多选或全部选择。无法获取空间名称时会停止同步，避免把群组 ID 当作文件夹名称。
5. 选定空间后，脚本显示该空间可访问的文件夹。输入 `0` 使用当前目录，输入文件夹序号进入子目录，输入 `b` 返回上一级；输入 `s` 或直接回车跳过目录选择。跳过后暂时使用该空间根目录，登录完成后可在网页设置中选择真正的 WebDAV 根目录。
...
省略 `--workspace-url` 时，脚本会在关闭浏览器后让你为唯一选中的 WPS 空间选择一个存储文件夹。输入 `0` 映射当前目录；选择子文件夹后，WebDAV 只从该文件夹开始显示；输入 `s` 或直接回车则暂时映射空间根目录，之后可在网页设置中选择。WPS 自动恢复的旧浏览器文件夹不会被自动使用。
