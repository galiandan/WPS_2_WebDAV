...
成功后目录中还会有 `wps-workspace.json`，其中保存账号类型、群组 ID、根目录 ID 和 WPS 返回的空间名称，不保存 Cookie。个人 WPS 会写入 `"mode":"personal"`；服务随后自动使用 `drive.wps.cn` 的个人接口。个人账号的 Cookie 由 `wps.cn` 域筛选，企业账号继续由 `kdocs.cn` 域筛选。
