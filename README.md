...
- 单个 WPS 空间：登录后按实时显示的空间名称选择一个，再选择一个 WebDAV 根目录。
...
预编译 Release 默认使用 `v0.9.95`。如果你维护自己的 Release 镜像，可在安装命令前设置 `WPS_ADAPTER_BINARY_BASE_URL`（目录地址，文件名由安装器追加）和 `WPS_ADAPTER_BINARY_RELEASE_TAG`。预编译资产名称为 `wps-adapter-linux-amd64`、`wps-adapter-linux-arm64` 等；当前没有对应资产时会自动进入源码回退路径。
...
2. 浏览器自动关闭，回到终端选择一个 WPS 空间；WebDAV 只能绑定一个空间，不能多选或全部选择。
3. 脚本显示该空间的文件夹。输入 `0` 使用当前目录，输入序号进入子文件夹，输入 `b` 返回上一级；输入 `s` 或直接回车可跳过目录选择，之后在网页“设置 → WebDAV 存储位置”中选择。已跳过时先临时使用该空间根目录。
4. 脚本验证当前空间并把 Cookie、CSRF 和工作区配置安全同步到 VPS。

WPS 登录后自动恢复的旧文件夹不会被误当成目标目录。WebDAV 始终只有一个明确的 WPS 空间和根目录；跳过目录时先映射该空间根目录，网页选择器随后可以把 `/dav/` 切换到其中的任意文件夹。切换位置不会移动 WPS 文件，只会改变 `/dav/` 的映射。脚本不会显示 Cookie、CSRF、密码或签名 URL，也不需要手动填写企业 ID、群组 ID 或文件夹 ID。
...
- WebDAV 每次只映射一个 WPS 空间；需要切换空间时重新运行登录助手，网页设置只负责选择当前空间内的目录。
