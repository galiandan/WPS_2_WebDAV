...
推荐直接使用仓库中的 `scripts/install-native.sh` 或 `scripts/install-docker.sh`；两者都支持 `--port PORT`、`--run-user USER`，默认使用执行 `sudo` 的当前用户，并且不会覆盖 `/etc/wps-adapter/secrets/`。安装器默认将群组和根目录设为 `auto`，由登录助手写入 `wps-workspace.json`。安装器会校验固定提交归档的文件清单；使用自定义 `--source-ref` 时还要提供对应的 `--source-manifest-sha256`。
